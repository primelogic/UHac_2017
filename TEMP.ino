...
byte resetPasswordCache; //new. temporary storage for changing password
byte confirmPasswordCache; //new. temporary storage for confirming password
bool passwordNotSet; //new.
uint8_t ACCOUNTING_STATE;

enum {NORMAL_OPER,SELECT_AMOUNT, EMERGENCY_MODE, ACCOUNT_ONES, ACCOUNT_TENS, TRANSACTION, TRANSACTION_FINISHED, NEW_DEVICE, CHANGE_PASSWORD, CONFIRM_PASSWORD}; //new CONFIRM_PASSWORD
...
switch(STATE){
  case NORMAL_OPER:
   ...
   case NEW_DEVICE:
    ...
    if (oneBtn.wasPressed()){bitWrite(passwordCache, 0, 1);}
    if (twoBtn.wasPressed()){bitWrite(passwordCache, 1, 1);}
    if (threeBtn.wasPressed()){bitWrite(passwordCache, 2, 1);}
    if (fourBtn.wasPressed()){bitWrite(passwordCache, 3, 1);} 
    EEPROM.write(100, 1);
   ...
   //pakita mo "Enter current password"
    if (oneBtn.wasPressed()){bitWrite(resetPasswordCache, 0, 1);}
    if (twoBtn.wasPressed()){bitWrite(resetPasswordCache, 1, 1);}
    if (threeBtn.wasPressed()){bitWrite(resetPasswordCache, 2, 1);}
    if (fourBtn.wasPressed()){bitWrite(resetPasswordCache, 3, 1);}
    
    if (oneBtn.wasReleased()|| 
        twoBtn.wasReleased()|| 
        threeBtn.wasReleased()|| 
        fourBtn.wasReleased()){
          if (resetPasswordCache == EEPROM.read(123)){STATE = CONFIRM_PASSWORD;}
          else {
   ...
   //pakita mo "Confirm your password"
    if (oneBtn.wasPressed()){bitWrite(confirmPasswordCache, 0, 1);}
    if (twoBtn.wasPressed()){bitWrite(confirmPasswordCache, 1, 1);}
    if (threeBtn.wasPressed()){bitWrite(confirmPasswordCache, 2, 1);}
    if (fourBtn.wasPressed()){bitWrite(confirmPasswordCache, 3, 1);}
    
    if (oneBtn.wasReleased()|| 
        twoBtn.wasReleased()|| 
        threeBtn.wasReleased()|| 
        fourBtn.wasReleased()){
          if (confirmPasswordCache == resetPasswordCache){
              //pakita mo "Password Changed!"
              EEPROM.write(50, confirmPasswordCache);
              STATE = NORMAL_OPER;
          }
          else {
